# Концепция аутентификации - Основные принципы

## Обзор

Система аутентификации HR Platform построена на принципах multi-device и multi-role поддержки, позволяя пользователям иметь несколько активных сессий в разных ролях на разных устройствах одновременно.
...
2. **HR роли внутри компании** (`hrRoleName` - из таблицы `hr_roles`):
   - Предустановленные роли (enum `HrRoleName`):
     ...
     - `HR_ADMIN` - HR администратор (может добавлять других HR)
   - Расширяемо через таблицу `hr_roles` (можно добавить HR_RECRUITER, HR_ANALYST и т.д. в БД)
...
1. **Один device = один refresh token = один активный role context**
   - Каждое устройство может иметь только одну активную сессию
   - При смене роли на устройстве - старый токен удаляется, создаётся новый

2. **Независимость устройств**
   - Login на новом устройстве не инвалидирует старые
   - Logout на одном устройстве не влияет на другие
   - Каждое устройство живёт своей жизнью

3. **Пример сценария:**
   ```
   Пользователь user@example.com:

   📱 Телефон: (U1, CANDIDATE, PHONE) - активен
   💻 Компьютер: (U1, HR, PC) - активен

   Оба устройства работают независимо.
   ```

### 4. Multi-Role поддержка

**Ключевые принципы:**

1. **Один email = несколько Role Context**
   - Пользователь может быть зарегистрирован как CANDIDATE
   - И одновременно как EMPLOYER (владелец компании)
   - Каждая роль имеет свой RoleContext

2. **Role Context в сессии**
   - В рамках одной сессии активна только одна роль
   - JWT всегда содержит `roleContextId` и `roleType`
   - Guards определяют доступ на основе role context

3. **Переключение роли**
   - Требуется новый login с выбором роли
   - Старый refresh token для этого device удаляется
   - Создаётся новый refresh token с новым role context

### 5. Sliding Session

**Принцип:**
- Access token продлевается при любой активности пользователя
- При каждом запросе с валидным access token проверяется время до истечения
- Если осталось < 50% времени - автоматически генерируется новый access token
- Refresh token также может продлеваться (опционально)

**Реализация:**
- Через Interceptor на защищённых маршрутах
- Прозрачно для клиента
- Улучшает UX (пользователь не замечает продления)

### 6. Автоматическая очистка токенов

**Cron задача:**
- Ежедневная очистка истёкших refresh токенов
- Удаление записей где `expiresAt < NOW()`
...
- `isActivated` - активирован ли email
- `activationLink` - ссылка для активации
- `createdAt`, `updatedAt`

**Принципы:**
- Один email может существовать только один раз
- Email используется для входа
- Пароль хранится в хэшированном виде (bcrypt/argon2)
...
1. **CANDIDATE**
   ...
   - Связан с CandidateProfile
   - Не привязан к компании

2. **EMPLOYER**
   - Работодатель (владелец компании или HR сотрудник)
   - Всегда привязан к компании через `companyId` (обязательно)
   - Может быть несколько EMPLOYER в одной компании
   - Владелец компании регистрируется как EMPLOYER с `hrRoleName: HR_ADMIN` (из таблицы hr_roles)
   - Типы работодателя (уровень внутри EMPLOYER):
     - **HR** - обычный HR сотрудник
     - **HR_ADMIN** - HR администратор, может добавлять других HR в компанию
     - В будущем можно добавить: HR_RECRUITER, HR_ANALYST и т.д.
...
**Важно:**
- Владелец компании = EMPLOYER с `hrRoleName: HR_ADMIN` (из таблицы hr_roles)
- Все работодатели работают через EMPLOYER роль
...
- `inn` - ИНН (опционально)
- `companyTypeId` - связь с `company_types` (позволяет добавлять новые типы без миграции)
- `ownerId` - User, который создал компанию (EMPLOYER с HR_ADMIN role)
- `createdAt`, `updatedAt`

**Связь с CompanyType:**
- Типы компаний хранятся в отдельной таблице `company_types`
- Позволяет добавлять новые типы без изменения схемы
- Типы: ORGANIZATION, IP, LAWYER, SELF_EMPLOYED, OTHER

**Принципы:**
- При создании компании владелец регистрируется как EMPLOYER с `hrRoleName: HR_ADMIN` (из таблицы hr_roles)
- Владелец компании = EMPLOYER с правами HR_ADMIN
- Компания может иметь несколько HR сотрудников
- Связана с CompanyType через `company_type_id`

### Token

Хранилище refresh токенов. Каждая запись = одна сессия на одном устройстве.

**Поля:**
- `id` - UUID
- `userId` - связь с User
- `roleContextId` - активный role context для этого токена
- `refreshToken` - хэшированный refresh token
- `deviceId` - уникальный идентификатор устройства
- `deviceName` - название устройства (опционально)
...
**Принципы:**
- Refresh token хранится в хэшированном виде
- Один device может иметь только один активный refresh token
- При смене роли на устройстве - старый токен удаляется
...
2. Проверяется User (существует, активирован, правильный пароль)
3. Если у пользователя несколько RoleContext - требуется выбор роли
...
- Проверка срока действия
- Сравнение hash
- Проверка deviceId (опционально, для дополнительной защиты)
...
2. **Token Theft**
   - Refresh token привязан к deviceId
   - Отслеживание IP адресов
   - Уведомления о новых устройствах

3. **Session Fixation**
   - Новый refresh token при каждом login
   - Удаление старых токенов при смене роли
...
- Hard delete (физическое удаление)
- Освобождает место в БД
- Не влияет на активные сессии

## Примеры использования

### Сценарий 1: Multi-device, одна роль

```
Пользователь user@example.com (CANDIDATE):

📱 Телефон: login → создаётся (U1, CANDIDATE, PHONE)
💻 Компьютер: login → создаётся (U1, CANDIDATE, PC)

Оба устройства работают независимо.
Logout на телефоне не влияет на компьютер.
```

### Сценарий 2: Multi-device, разные роли

```
Пользователь user@example.com:

📱 Телефон: login as CANDIDATE → (U1, CANDIDATE, PHONE)
💻 Компьютер: login as EMPLOYER → (U1, EMPLOYER, PC)

Оба устройства работают независимо.
На телефоне - роль кандидата, на компьютере - EMPLOYER (владелец компании).
```

### Сценарий 3: Смена роли на устройстве

```
Пользователь user@example.com:

💻 Компьютер: login as CANDIDATE → (U1, CANDIDATE, PC)
💻 Компьютер: logout
💻 Компьютер: login as EMPLOYER → (U1, EMPLOYER, PC)

Старый токен (CANDIDATE) удаляется.
Создаётся новый токен (EMPLOYER).
```

### Сценарий 4: HR в компании
...
### Сценарий 5: Регистрация HR в существующую компанию

```
HR-ADMIN регистрирует нового HR:
POST /auth/register/hr
{
  email: "hr@company.com",
  password: "Password123",
  companyId: "company-id",
  hrRoleName: "HR" // или "HR_ADMIN" (из таблицы hr_roles, для предустановленных совпадает с HrRoleName.HR или HrRoleName.HR_ADMIN)
}
...
Система аутентификации HR Platform обеспечивает:

✅ Безопасную multi-device поддержку
✅ Multi-role контекст для одного пользователя
✅ Независимые сессии на разных устройствах
✅ Автоматическое управление токенами
✅ Защиту от основных типов атак
✅ Двухуровневую систему ролей:
   - Глобальный уровень: CANDIDATE | EMPLOYER | ADMIN
   - Внутри EMPLOYER: HR | HR_ADMIN (масштабируемо)
✅ Типы компаний в отдельной таблице (масштабируемо)
✅ Иерархию ролей работодателя (HR, HR_ADMIN)
✅ Регистрацию HR в существующую компанию (только HR_ADMIN)
✅ companyId в JWT для EMPLOYER ролей (обязательно для EMPLOYER)
✅ Проекты компаний для группировки вакансий (Company → Project → Vacancy)

Все потоки детально описаны в соответствующих разделах документации.
